...
janela.title('Colecao de Notas e Moedas')
janela.geometry('1045x800')
janela.configure(background = 'black')
#janela.resizable(False,False)
conteudo = Frame(janela, width=900, height=700)
conteudo = Frame(janela, width=900, height=700)
conteudo.grid(row=0,column=0,sticky=W+E+N+S)

label = Label(conteudo, text='Pesquisa na Colecao de notas/moedas', padx=2, pady=2)
...
radioValor = IntVar()
rb_moeda = Radiobutton(selecao, text="Moeda", variable=radioValor, value=1)
rb_nota = Radiobutton(selecao, text="Notas", variable=radioValor, value=2)
rb_moeda.grid(row=0, column=0)
rb_nota.grid(row=0, column=1)

rb_valor = Label(conteudo, textvariable=radioValor, padx=2, pady=2)
rb_valor.grid(row=0, column=1)

label = Label(conteudo, text='Pesquisar por Nota ou Moeda?', padx=2, pady=2)
label.grid(row=1, column=0)

label1 = Label(conteudo, text='O que procura?', padx=2, pady=2)
